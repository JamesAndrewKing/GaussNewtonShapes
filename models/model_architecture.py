...
class PlaceholderNet(nn.Module):
    ...
    def _r_gauss_curvature(self, params, x):
        grad_f = self._grad_x_f(params, x).squeeze(1)
        hess_f = self._hess_x_f(params, x).squeeze(1)
    ...
    def r_gauss_curvature(self, params, x):
        return vmap(self._r_gauss_curvature, in_dims=(None, 0), out_dims=(0))(params, x.to(dtype=torch.float64))

    def _grad_theta_r_gauss_curvature(self, params, x):
        return jacrev(self._r_gauss_curvature, argnums=0)(params, x.to(dtype=torch.float64))

    def grad_theta_r_gauss_curvature(self, params, x):
        return vmap(self._grad_theta_r_gauss_curvature, in_dims=(None, 0), out_dims=(0))(params, x.to(dtype=torch.float64))

    def _r_principle_curvature_1(self, params, x):
        k_m = self._r_mean_curvature(params, x)
        k_g = self._r_gauss_curvature(params, x)
