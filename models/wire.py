...
class ConditionalWIRE(nn.Module):
    ...
    def forward(self, x, z=0.05):
        if torch.is_tensor(z):
            if len(x.shape)==2:
                assert len(z)==len(x) and x.shape[1] + z.shape[1]==self.layers[0], f"z has bad shape {z.shape} for x with shape {x.shape}"
            elif len(x.shape)==1:
                assert x.shape[0] + z.shape[0]==self.layers[0], f"z has bad shape {z.shape} for x with shape {x.shape}"
            xz = torch.cat([x, z], dim=-1)
        elif type(z) in [float, int]:
            xz = torch.cat([x, z + torch.zeros(len(x), 1, dtype=x.dtype)], dim=-1) ## hacky
        output = self.net(xz)
        return output