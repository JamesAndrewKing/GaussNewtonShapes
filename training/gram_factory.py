...
def compute_residual(model, config):
    ...
    if "pts_surface" in config and loss_weights.get("gauss_curvature", 0.0) > 0:
        pts = config["pts_surface"]
        N = pts.shape[0]
        r = model.r_gauss_curvature(params, pts).squeeze(1)
        r_blocks.append(
    ...
    for layer_name in model.params:
        if 'weight' in layer_name or 'bias' in layer_name:
            ...
            if "pts_surface" in config and loss_weights.get("gauss_curvature", 0.0) > 0:
                pts = config["pts_surface"]
                N = pts.shape[0]
                dr = flatten_layer_grads(model.grad_theta_r_gauss_curvature(layer_params, pts), layer_name)
    ...
    for layer_name in model.params:
        if 'weight' in layer_name or 'bias' in layer_name:
            ...
            if "pts_surface" in config and loss_weights.get("gauss_curvature", 0.0) > 0:
                pts = config["pts_surface"]
                N = pts.shape[0]
                grad = flatten_layer_grads(model.grad_theta_r_gauss_curvature(layer_params, pts), layer_name)
